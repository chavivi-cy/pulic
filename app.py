...
if sel_q == qs[0]:
    ...
    # 拆解变量，防止报错
    trace1 = go.Bar(name='回收成本', x=['P&L'], y=[buyback_v], marker_color=JP_COLORS[0])
    trace2 = go.Bar(name='整备增值', x=['P&L'], y=[refurb_c+480], base=buyback_v, marker_color=JP_COLORS[1])
    trace3 = go.Bar(name='净利润', x=['P&L'], y=[profit], base=buyback_v+refurb_c+480, marker_color=JP_COLORS[2])
    fig = go.Figure(data=[trace1, trace2, trace3])
    st.plotly_chart(fig, use_container_width=True)

elif sel_q == qs[1]:
    st.write("### Q2: 商业目标 - 拉新与留存 (旭日图)")
    data_q2 = {"A": ["拉新", "拉新", "留存", "留存"], "B": ["新入iOS", "安卓切换", "旧机换新", "服务增购"], "V": [20, 15, 45, 20]}
    df2 = pd.DataFrame(data_q2)
    fig = px.sunburst(df2, path=['A','B'], values='V', color_discrete_sequence=[JP_COLORS[0], JP_COLORS[3]])
    st.plotly_chart(fig, use_container_width=True)

elif sel_q == qs[2]:
    st.write("### Q3: KSF - 技术确权维度图")
    data_q3 = {"r": [98, 95, 99, 88, 92], "theta": ['部件配对', 'SN溯源', '激活校验', 'ATE测试', '定价权']}
    df3 = pd.DataFrame(data_q3)
    ...
    y_data = ["回收总量 (100%)", "通过初检 (85%)", "原厂重整 (80%)", "合格成品 (78%)"]
    x_data = [base_vol, base_vol*0.85, base_vol*0.80, base_vol*0.78]
    fig = go.Figure(go.Funnel(y=y_data, x=x_data, marker={"color": JP_COLORS}, textinfo="value+percent initial"))
    st.plotly_chart(fig, use_container_width=True)

elif sel_q == qs[4]:
    st.write("### Q5: 渠道份额详细分布")
    data_q5 = {"渠道": ["京东自营", "爱回收", "官网(iPad/Mac)", "转转及其他", "B2B集采"], "占比": [45, 20, 15, 10, 10], "父级": ["所有渠道"] * 5}
    df5 = pd.DataFrame(data_q5)
    ...
    data_q6 = {"画像受众": ["精致白领 (35%)", "数码极客 (25%)", "在校学生 (25%)", "小镇青年 (15%)"], "占比": [35, 25, 25, 15]}
    df6 = pd.DataFrame(data_q6)
    ...
    rv_data = [95,85,71,65,58,52,45, 92,80,65,50,42,35,28, 88,75,55,45,38,30,22, 80,55,40,28,18,10,5]
    brand_data = ["Apple"]*7 + ["Huawei"]*7 + ["Samsung"]*7 + ["安卓平均"]*7
    df7 = pd.DataFrame({"月": m_list*4, "RV": rv_data, "B": brand_data})
    ...
    fig = px.scatter(x=[90, 85, 75], y=[95, 80, 70], text=["隐私安全","品牌溢价","售后纠纷"], size=[40, 25, 30], color_discrete_sequence=[JP_COLORS[2]], labels={'x':'X：风险发生概率', 'y':'Y：负面冲击程度'})
    st.plotly_chart(fig, use_container_width=True)

st.markdown("---")

# ---------------------------------------------------------
# 5. 流转全景图 (Sankey) - 绝对安全版
# ---------------------------------------------------------
st.header("🌐 中国区逆向流转全景")
...
# 5 -> 6-10
source_grp3 = [5, 5, 5, 5, 5]
target_grp3 = [6, 7, 8, 9, 10]
value_grp3 = [45, 20, 15, 10, 10]

# 合并列表
all_sources = source_grp1 + source_grp2 + source_grp3
all_targets = target_grp1 + target_grp2 + target_grp3
all_values = value_grp1 + value_grp2 + value_grp3

# 4. 构建图表对象 (分离定义，杜绝嵌套错误)
# 节点配置 (黑色字体)
node_config = dict(
    pad=40, thickness=25, 
...
# 创建 Figure
sankey_obj = go.Sankey(node=node_config, link=link_config)
fig_s = go.Figure(data=[sankey_obj])
